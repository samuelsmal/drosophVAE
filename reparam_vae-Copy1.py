...
!ls /ramdya-nas/SVB/experiments/180918_MDN_CsCh/Fly3/003_SG1/behData/images | grep pkl
...
np.linspace(8, 30 * 2, 2 * 4).astype(np.int).tolist() + [30]
...
class R_VAE(tf.keras.Model):
    def __init__(self, latent_dim, input_shape, batch_size, n_layers=3, dropout_rate_temporal=0.2):
        ...
        self._layer_sizes_inference  = np.linspace(input_shape[-1] * 2, 2 * latent_dim, n_layers).astype(np.int)
        ...
        self._layer_sizes_generative = np.linspace(latent_dim, input_shape[-1] * 2, 2 * n_layers).astype(np.int).tolist() + [input_shape[-1]]
